 ...
 * 
 */
package com.mymercury.dataprovider;
...
import com.mymercury.utility.NewExcelLibrary;
...
public class DataProviders {
	...
	//Class --> Data Source, EndToEndTest	
		@DataProvider(name = "DataSource")
		public Object[][] getProduct() {
			// Totals rows count
			int rows = obj.getRowCount("DataSource");
			// Total Columns
			int column = obj.getColumnCount("DataSource");
			int actRows = rows - 1;
			...
			for (int i = 0; i < actRows; i++) {
				for (int j = 0; j < column; j++) {
					data[i][j] = obj.getCellData("DataSource", j, i + 2);
				}
			}
			return data;
		}
